...
async function combinedFlow({
  ...
  try {
    ...
    // Helper to ensure currency is set to USD
    async function ensureCurrencyIsUSD() {
      onProgress('Setting currency to USD...');
      if (isBlazor) {
        const currencyDropdown = frame.locator('.order-details-currency-dropdown').first();
        if (await currencyDropdown.isVisible().catch(() => false)) {
          let currentCurrency = await currencyDropdown.innerText().catch(() => '');
          currentCurrency = currentCurrency.trim();
          if (currentCurrency !== 'USD') {
            onProgress(`Current currency is "${currentCurrency}". Changing to "USD"...`);
            await currencyDropdown.click();
            await page.waitForTimeout(1000);
            
            const usdOption = frame.locator('.k-list-item, [role="option"], li').filter({ hasText: /^USD$/i }).first();
            if (await usdOption.isVisible().catch(() => false)) {
              await usdOption.click();
              await page.waitForTimeout(1500);
            } else {
              const usdOptionPartial = frame.locator('.k-list-item, [role="option"], li').filter({ hasText: 'USD' }).first();
              if (await usdOptionPartial.isVisible().catch(() => false)) {
                await usdOptionPartial.click();
                await page.waitForTimeout(1500);
              } else {
                onProgress('Warning: USD option not found in currency dropdown.');
              }
            }
          } else {
            onProgress('Currency is already USD. No action needed.');
          }
        } else {
          onProgress('Warning: Currency dropdown not visible or found.');
        }
      } else {
        const jqueryCurrency = frame.locator('select#Currency, select[name="Currency"], select[name*="currency" i], select#currency').first();
        if (await jqueryCurrency.isVisible().catch(() => false)) {
          onProgress('Setting currency to USD (jQuery)...');
          await jqueryCurrency.selectOption({ value: 'USD' }).catch(async () => {
            await jqueryCurrency.selectOption({ label: 'USD' });
          });
          await page.waitForTimeout(1000);
        }
      }
    ...
    if (items && Array.isArray(items) && items.length > 0) {
      ...
      if (isBlazor) {
        ...
        // Ensure currency is set to USD (now visible since items section is expanded)
        await ensureCurrencyIsUSD();

        // Get count of existing items in the Blazor grid
        const editBtns = frame.locator('button.grid-button-edit');
        const existingRowsCount = await editBtns.count();
        onProgress(`Found ${existingRowsCount} existing items in DHL Blazor grid.`);

        // Check if existing items already match the target payload
        const existingItems = await frame.evaluate(() => {
          ...
          return rows.map(row => {
            ...
            const sku = inputs[1] ? (inputs[1].value || '').trim() : '';
            const quantity = inputs[2] ? parseFloat(inputs[2].value) || 0 : 0;
            const price = inputs[3] ? parseFloat(inputs[3].value) || 0 : 0;
            const weight = inputs[4] ? parseFloat(inputs[4].value) || 0 : 0;
            return { sku, name, quantity, price, weight };
          });
        });
        ...
        if (existingItems && existingItems.length === items.length) {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            const targetWeight = Number(target.weight !== undefined && target.weight !== null && target.weight !== '' ? target.weight : 0.001);
            
            if (String(target.sku || '').trim().toLowerCase() !== String(current.sku || '').trim().toLowerCase() ||
                String(target.name || '').trim().toLowerCase() !== String(current.name || '').trim().toLowerCase() ||
                Number(target.quantity || 1) !== Number(current.quantity) ||
                Math.abs(Number(target.price || 0.1) - Number(current.price)) > 0.01 ||
                Math.abs(targetWeight - Number(current.weight)) > 0.001) {
        ...
        if (alreadyMatches) {
          onProgress('Items in Blazor grid already match the target payload. Skipping items update.');
        } else {
          // Read customs details from the first existing item (if any) to use as default/fallback for new items
          let fallbackHsCode = '';
          let fallbackOrigin = 'israel';

          if (existingRowsCount > 0) {
            onProgress('Inspecting the first existing item to capture customs details (HS Code / Country of Origin)...');
            ...
            const hsCodeInput = modal.locator('input[placeholder*="HS Code"]').first();
            const originInput = modal.locator('input[placeholder*="country of origin"]').first();

            fallbackHsCode = await hsCodeInput.inputValue().catch(() => '');
            fallbackOrigin = await originInput.inputValue().catch(() => 'israel') || 'israel';

            onProgress(`Captured fallback HS Code: "${fallbackHsCode}", Country of Origin: "${fallbackOrigin}"`);

            // Close the modal
            const closeBtn = modal.locator('button.btn-modal-close, button:has-text("Cancel"), button:has-text("Close")').first();
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            if (i < existingRowsCount) {
              // Update existing row
              onProgress(`Updating existing item row at index ${i} with SKU "${item.sku}"...`);
              const editBtns = frame.locator('button.grid-button-edit');
            ...
            if (await qtyToShipInput.isVisible().catch(() => false)) {
              ...
              await qtyToShipInput.press('Tab');
              await page.waitForTimeout(500);
            }
            ...
            const itemWeight = String(item.weight !== undefined && item.weight !== null && item.weight !== '' ? item.weight : '0.001');
            await weightInput.fill(itemWeight);
            await weightInput.press('Tab');
            await page.waitForTimeout(500);

            // Fill Customs details to pass international validation
            const targetHsCode = item.hsCode || item.hs_code || item.harmonized_code || fallbackHsCode;
            if (targetHsCode) {
              onProgress(`Filling HS Code: "${targetHsCode}"`);
              await hsCodeInput.fill(String(targetHsCode));
              await hsCodeInput.press('Tab');
              await page.waitForTimeout(500);
            }

            const targetOrigin = item.countryOfOrigin || item.country_of_origin || item.coo || fallbackOrigin;
            if (targetOrigin) {
              onProgress(`Filling Country of Origin: "${targetOrigin}"`);
              await originInput.fill(String(targetOrigin));
          ...
          // Delete any leftover existing items
          if (existingRowsCount > items.length) {
            const deleteCount = existingRowsCount - items.length;
            onProgress(`Deleting ${deleteCount} leftover items from the order...`);
            
            for (let i = 0; i < deleteCount; i++) {
              // Always delete the row at index `items.length` because rows shift up when deleted
              onProgress(`Deleting leftover item at row index ${items.length}...`);
              const row = frame.locator('.order-section-content.items-section tr.k-master-row, .items-section tr.k-master-row').nth(items.length);
        ...
        const existingItems = await frame.evaluate(() => {
          ...
          let idx = 0;
          while (true) {
            const skuInput = document.getElementById(`item-SKU-${idx}`);
            if (!skuInput) break;
            const nameInput = document.getElementById(`item-Description-${idx}`);
            const qtyInput = document.getElementById(`item-Quantity-${idx}`);
            const priceInput = document.getElementById(`item-UnitPrice-${idx}`);
            const weightInput = document.getElementById(`item-Weight-${idx}`);
            ...
            itemsList.push({ sku, name, quantity, price, weight });
            idx++;
          }
          return itemsList;
        });
        ...
        if (alreadyMatches) {
          onProgress('Items in jQuery grid already match the target payload. Skipping items update.');
        } else {
          ...
          for (const targetItem of items) {
            ...
            const itemWeight = String(targetItem.weight !== undefined && targetItem.weight !== null && targetItem.weight !== '' ? targetItem.weight : '0.001');
            await weightInput.fill(itemWeight);
    ...
    // Calculate total weight of the items to use for package weight
    let targetPackageWeight = '0';
    if (items && Array.isArray(items) && items.length > 0) {
      const sumWeight = items.reduce((sum, it) => {
        const w = parseFloat(it.weight !== undefined && it.weight !== null && it.weight !== '' ? it.weight : 0.001);
        const q = parseInt(it.quantity) || 1;
        return sum + (w * q);
      }, 0);
      targetPackageWeight = String(sumWeight);
    } else {
      // If no items payload was passed, calculate the sum of the weights of the existing items on the page
      targetPackageWeight = await frame.evaluate((isBlazorLayout) => {
        if (isBlazorLayout) {
          const rows = Array.from(document.querySelectorAll('.order-section-content.items-section tr.k-master-row, .items-section tr.k-master-row'));
          const sum = rows.reduce((acc, row) => {
            const inputs = Array.from(row.querySelectorAll('input'));
            const qty = inputs[2] ? parseFloat(inputs[2].value) || 1 : 1;
            const weight = inputs[4] ? parseFloat(inputs[4].value) || 0 : 0;
    ...
    // Sanitization check for targetPackageWeight
    let parsedWeight = parseFloat(targetPackageWeight) || 0.001;
    if (parsedWeight <= 0) parsedWeight = 0.001;
    targetPackageWeight = String(parsedWeight);
    ...
    // 3. Set overall package weight dynamically
    onProgress(`Setting overall package weight (Place #1) to: ${targetPackageWeight} forcefully...`);
    ...
    if (await overallWeightInput.isVisible().catch(() => false)) {
      ...
      await overallWeightInput.fill(targetPackageWeight);
    ...
    if (incoterms) {
      ...
      // Duties & Taxes Paid checkbox logic based on Incoterms DDP
      const shouldCheckDtp = incoterms.toLowerCase() === 'ddp';
      onProgress(`Duties & Taxes Paid option should be ${shouldCheckDtp ? 'checked' : 'unchecked'} based on incoterms "${incoterms}"...`);
      
      const dtpCheckbox = frame.locator('#DTP').first();
      if (await dtpCheckbox.count() > 0) {
        if (!(await dtpCheckbox.isVisible().catch(() => false))) {
          onProgress('Options section appears collapsed. Clicking Options header to expand...');
          const optionsHeader = frame.locator('.order-section-title:has-text("Options"), .order-section-heading:has-text("Options"), *:has-text("Options")').first();
          if (await optionsHeader.isVisible().catch(() => false)) {
        ...
        if (await dtpCheckbox.isVisible().catch(() => false)) {
          const isChecked = await dtpCheckbox.isChecked().catch(() => false);
          if (shouldCheckDtp && !isChecked) {
            onProgress('Checking Duties & Taxes Paid checkbox...');
            await dtpCheckbox.click({ force: true });
            await page.waitForTimeout(500);
          } else if (!shouldCheckDtp && isChecked) {
            onProgress('Unchecking Duties & Taxes Paid checkbox...');
            await dtpCheckbox.click({ force: true });
            await page.waitForTimeout(500);
          } else {
            onProgress(`Duties & Taxes Paid is already ${isChecked ? 'checked' : 'unchecked'}. No action needed.`);
          }
        } else {
          onProgress('Warning: Duties & Taxes Paid (#DTP) checkbox is not visible.');
        }
      } else {
        onProgress('Warning: Duties & Taxes Paid (#DTP) checkbox not found.');
      }
    }
    ...
    if (skipPrint) {
      onProgress('Save-only option enabled. Closing browser and skipping label printing.');
      const result = {
        orderId: orderNumber,
        invoiceURL,
        ...
        labelFile: null,
        status: 'completed',
        message: 'Invoice uploaded and order details saved successfully (print skipped)'
      };
      ...
      onProgress('Save-only flow completed.');
      return result;
    }
