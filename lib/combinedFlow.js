...
async function combinedFlow({
  ...
  try {
    ...
    // Set overall package weight if weight is provided (Place #1)
    ...
    if (weight) {
      ...
      if (await overallWeightInput.isVisible().catch(() => false)) {
        ...
        onProgress('Warning: Overall package weight input (#Weight) not visible or not found.');
      }
    }

    // Insurance update is disabled per client request
    ...
    if (items && Array.isArray(items) && items.length > 0) {
      ...
      const isBlazor = page.url().includes('/orders/new');
      ...
      if (isBlazor) {
        ...
        // Add a single item
        onProgress('Clicking Add Item button...');
        await addItemBtn.click();
        await page.waitForTimeout(2000); // Wait for Blazor row to enter edit mode

        // Fill inputs for index 0 (Place #2 for weight)
        ...
        await weightInput.fill(String(targetItem.weight || '0.001'));
